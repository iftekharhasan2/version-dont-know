...
const ALL_CARDS: CardData[] = [
  ...
  },
  {
    ...
    subtitle: 'Sub-Atomic Defense Lattice',
    description: 'Autonomous neural node regulating quantum entanglement coherence across multi-regional arrays.',
    badge: 'MYTHIC',
    backgroundUrl: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?q=80&w=800&auto=format&fit=crop',
    shadowUrl: 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?q=80&w=800&auto=format&fit=crop',
    frameColor: '#38bdf8',
  },
  {
    id: 'solar-alchemist',
    title: 'Solar Alchemist',
    subtitle: 'Helios Convergent Matrix',
    description: 'Synthesizing plasma isotopes into high-density clean propellant fuels for deep orbital habitats.',
    badge: 'RARE',
    backgroundUrl: 'https://images.unsplash.com/photo-1534447677768-be436bb09401?q=80&w=800&auto=format&fit=crop',
    shadowUrl: 'https://images.unsplash.com/photo-1534447677768-be436bb09401?q=80&w=800&auto=format&fit=crop',
    frameColor: '#f97316',
  },
  {
    id: 'abyssal-leviathan',
    title: 'Abyssal Leviathan',
    subtitle: 'Hadopelagic Trench Core',
    description: 'Deep-ocean geothermal harvester navigating extreme hydrothermal pressure zones.',
    badge: 'LEGENDARY',
    backgroundUrl: 'https://images.unsplash.com/photo-1682687220063-4742bd7fd538?q=80&w=800&auto=format&fit=crop',
    shadowUrl: 'https://images.unsplash.com/photo-1682687220063-4742bd7fd538?q=80&w=800&auto=format&fit=crop',
    frameColor: '#06b6d4',
  },
  {
    id: 'astral-weaver',
...
export const ParallaxCardsSection: React.FC<ParallaxCardsSectionProps> = ({ embedded = false }) => {
  ...
  const [cardScale, setCardScale] = useState<number>(0.88);
  const containerRef = useRef<HTMLDivElement>(null);

  // Dynamically scale card amount and scale according to screen/container size
  useEffect(() => {
    const updateResponsiveSizing = () => {
      const containerWidth = containerRef.current?.getBoundingClientRect().width || 0;
      const effectiveWidth = containerWidth > 100 ? containerWidth : window.innerWidth;

      if (effectiveWidth < 540) {
        // Mobile screens (<540px): 1 card, fits perfectly in 1 row
        setCardsPerPage(1);
        setCardScale(0.82);
      } else if (effectiveWidth < 820) {
        // Large phones & compact tablets (540px - 820px): 2 cards in 1 row
        setCardsPerPage(2);
        setCardScale(0.78);
      } else if (effectiveWidth < 1140) {
        // Tablets & small desktops (820px - 1140px): 3 cards in 1 row
        setCardsPerPage(3);
        setCardScale(0.80);
      } else if (effectiveWidth < 1480) {
        // Standard desktops / laptops (1140px - 1480px): 4 cards in 1 row
        setCardsPerPage(4);
        setCardScale(0.84);
      } else if (effectiveWidth < 1820) {
        // Large monitors (1480px - 1820px): 5 cards in 1 row
        setCardsPerPage(5);
        setCardScale(0.85);
      } else {
        // Ultra-wide / 4K monitors (>=1820px): 6 cards in 1 row
        setCardsPerPage(6);
        setCardScale(0.88);
      }
    };
    ...
    const handleResize = () => {
      ...
      resizeTimer = setTimeout(updateResponsiveSizing, 100);
    };
    ...
    <div className={`w-full ${embedded ? 'pt-0' : 'max-w-7xl mx-auto pt-2'} space-y-5`}>
      ...
      {filteredCards.length === 0 ? (
        ...
        <div className="space-y-5">
          <div 
            ref={containerRef}
            className={`${embedded ? 'bg-[#050a12]/60' : 'bg-[#081220]/90'} border border-slate-800 rounded-3xl p-3.5 sm:p-5 md:p-6 shadow-inner overflow-hidden w-full`}
          >
            {/* 3D Cards Single-Row Container: always stays in 1 row across every screen size with compact gap */}
            <div className="flex flex-nowrap justify-center items-center gap-1.5 sm:gap-2 md:gap-2.5 py-2 px-1 w-full overflow-x-auto no-scrollbar scroll-smooth">
